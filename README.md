...
当前已推进到 Step 8，完成当前用户会话列表、加载、重命名、删除与分层重构。
...
- 完善当前用户会话列表、加载、重命名和删除。
- 所有列表选择继续使用从 1 开始的临时序号。
- 删除会话前二次确认，删除当前会话后清空 TUI 状态。
- 清理 UI 对存储后端的分层穿透，业务规则集中到 `SessionManager`。
...
已建立基础工程结构、Pydantic 数据模型、异步存储接口、配置管理、TUI 主菜单骨架、SQLite 存储后端、用户管理菜单、预设管理菜单、无状态对话引擎、TUI 多轮流式对话视图和当前用户会话管理功能。对话搜索留到 Step 9，导出和模型切换留到 Step 10。
