...
> 本文用于课程答辩中的“AI 辅助开发记录与 Vibe Coding 能力证明”。以下总览是根据现有日志整理出的评测化摘要，不是原始 Prompt 原文。现有日志没有完整保存全部原始 Prompt、逐轮对话和每轮修改次数，因此本文不倒推出“当时的完整 Prompt”，也不虚构精确交互轮次。

## 1. 本次整理前核对记录

| 核对项 | 结果 |
| --- | --- |
| `git status --short` | 无输出，开始整理前工作区干净 |
| Git 根目录 | `D:/project/langchain-chat` |
| 当前分支 | `main` |
| 已完整读取 | `docs/ai_usage_log.md` |
| 已读取用于事实核对 | `docs/architecture.md` |
| 仓库内不存在 | `docs/需求说明文档.md`、`docs/需求变更与扩展登记.md` |
| 本次未执行 | `git add`、`git commit`、`git tag`、`git push` |
...
| 项目名称 | `langchain-chat` |
| AI 工具 | Codex |
| 开发方式 | 用户给出目标、边界、报错和验证要求，Codex 进行实现、调试、验证和日志记录的 Vibe Coding 协作方式 |
| 正式实施步骤数量 | 15 个正式 Step：Step 1 至 Step 15 |
| 补充修复记录数量 | 2 个，均归属 Step 5 后续纠偏 |
| 当前自动化测试状态 | 日志可精确确认的最近完整测试数量为 `67 passed, 1 skipped`；Step 15 记录了新增配置测试和自动验证，但未在日志正文中保留最终精确 passed/skipped 数量 |
| 当前最终验收状态 | 主体功能、工程化配置和默认自动化测试已基本完成；完整 TUI 人工流程、真实 LLM、真实 MySQL、prod 冒烟和跨环境真实业务流程仍需人工或外部环境验证 |
| 日志覆盖范围 | Step 1 项目初始化至 Step 15 dev/test/prod 多环境配置隔离；包含 Step 5 的编号系统和数据库主键展示补充修复 |
| 日志数据限制 | 未保存全部原始 Prompt、完整交互轮次、每轮输出和所有人工反馈，因此不能精确计算平均功能达成轮次、Prompt 字数、严格死循环解脱率等指标 |

## 3. 四项能力评测摘要

| 评测维度 | 日志中的主要证据 | 可量化指标 | 数据限制 | 当前评价 |
| --- | --- | --- | --- | --- |
| 需求拆解与架构能力 | 项目被拆分为 15 个正式 Step；先抽象 `StorageBackend`、`AbstractUI`、配置模型，再实现 SQLite/MySQL/File 等具体后端；`ChatEngine` 保持无状态；业务层依赖 `StorageBackend`；UI、业务、存储分层；WebUI、多模型并行对比、图文、语音、Tool Calling 仅预留接口；多环境配置延后到 Step 15 实现 | 正式 Step 15 个；补充修复 2 个；三种存储后端；dev/test/prod 三环境 | 原始需求文档未全部在仓库内保留；部分步骤的原始 Prompt 不完整 | 能体现从骨架、数据层、业务层、UI、LLM、存储扩展、测试、文档到环境隔离的阶段化拆解 |
| 提示词工程与精准度 | 多次记录工作目录限制、技术栈限制、允许和禁止修改范围、不创建真实 `.env`、不提前实现后续功能、明确验证命令；用户提供真实报错、命令输出、输入输出示例后推动纠偏 | 日志中每个 Step 都有目标、约束或验证命令；至少 10/17 个条目显式包含修复、重构、测试、安全或隔离目标 | 原始 Prompt 和完整交互轮次没有全部保留，因此不能精确计算平均功能达成轮次 | 约束表达较清晰，能减少越界实现和敏感信息泄露；但不能把整理后的约束摘要当成原始 Prompt 原文 |
| 逆境调试与抗幻觉能力 | 记录了 `Preset(id=0)` 判断问题、缺少单个预设查询接口、数据库主键直接作为展示编号、UI 直接访问 backend、模型配置来源不一致、测试读取真实 `.env` 或真实模型、prod MySQL 配置错误回退风险等问题 | 已记录缺陷 7 个，已关闭 7 个；已记录缺陷关闭率为 7/7 | 该数据是缺陷闭环情况，不等同于完整原始对话计算出的死循环解脱率；AI 连续错误次数无法还原 | 能证明在专项检查、回归测试和用户反馈下持续纠偏，并且没有把后续修复伪装成首次正确 |
| 代码审查与安全防范能力 | `.env` 和运行数据不提交；SQL 参数化；数据库名称白名单；用户隔离；会话和消息归属校验；日志脱敏；不记录 API Key、密码和完整 Prompt；临时文件加 `os.replace`；JSON 损坏时不静默覆盖；prod 不回退 dev；环境变量和 dotenv 隔离；测试使用临时数据库；UI 不访问具体后端 | 安全或隔离相关条目至少覆盖 Step 1、5、7、8、9、11、12、13、15 | 安全验证主要来自自动化测试、静态检查和日志记录；真实生产 MySQL 与真实 LLM 未完成端到端验收 | 已形成较完整的工程安全边界，但不能表述为生产环境安全已全部验收 |

## 4. 已记录问题闭环表

| 问题 | 根本原因 | 纠偏方向 | 验证结果 |
| --- | --- | --- | --- |
| `Preset(id=0)` 保存判断错误 | 新建模型使用 `id=0`，旧逻辑只判断 `id is None` | 使用 `not preset.id` 判断新增，并要求更新不存在 ID 时明确失败 | Step 5 临时数据库验证通过，`Preset(id=0)` 保存后 ID 大于 0 |
| 缺少单个预设查询接口 | `StorageBackend` 只有列表查询，业务层需要按 ID 获取预设 | 新增 `get_preset_by_id` 抽象方法，SQLite 用参数化 SQL 实现 | Step 5 验证 `get_preset_by_id` 可查询 |
| 数据库主键直接作为展示编号 | TUI 把 `preset.id` 当成用户可见编号 | 引入每个列表局部的展示序号，并映射回真实数据库 ID | Step 5 补充修复 A 通过 compileall、init_db、TUI 启动、Ruff、Pytest |
| 数据库 ID 暴露到 TUI 表头 | 展示层仍显示内部持久化标识 | 隐藏“数据库 ID”列，只展示序号、名称等用户信息 | Step 5 补充修复 B 确认表头不再包含“数据库 ID” |
| UI 直接访问 backend | 分层边界被穿透，TUI 依赖具体存储细节 | 将会话归属、加载、重命名、删除等收口到 `SessionManager` | Step 8 使用 `Select-String "\.backend\."` 检查 `src\ui` 无输出 |
| 模型配置来源不一致 | 用户默认模型、新建会话模型和 `ChatEngine` 来源不统一 | 统一为 `config.model_name`，旧数据不自动迁移 | Step 9 配置源检查确认 `config.model_name` 与 `ChatEngine.model_name` 一致 |
| 测试读取真实 `.env` 或构造真实模型 | 测试未隔离配置和模型边界 | 使用 `_env_file=None`、monkeypatch 和 FakeModel | Step 13 默认测试不调用真实 LLM，也不读取真实 `.env` |
| prod MySQL 配置可能错误回退 | 旧环境解析会使用通用默认值掩盖缺失生产变量 | `PROD_MYSQL_*` 必须显式提供，prod 不回退 dev 或旧 `.env` | Step 15 新增配置隔离测试并记录该缺陷已修复 |
...
| 指标 | 数值或状态 | 统计口径 |
| --- | --- | --- |
| 正式 Step 数量 | 15 | Step 1 至 Step 15 |
| 补充修复数量 | 2 | Step 5 补充修复 A、B |
| 日志条目总数 | 17 | 15 个正式 Step + 2 个补充修复 |
| 最新精确测试数量 | `67 passed, 1 skipped` | 日志中 Step 13 最终验收和 Step 14 前置状态明确记录 |
| 已记录缺陷数量 | 7 | 合并同类后统计明确记录且已定位的问题；上表列出 8 行，其中数据库 ID 展示相关两行视为同一类问题的连续修复 |
| 已关闭缺陷数量 | 7 | 现有日志中均有修复方向和验证记录 |
| 明确包含修复、重构、测试、安全或隔离目标的条目 | 至少 10/17 | 按标题、任务目标或“发现并修复”记录统计，保守口径 |

### 不能直接统计的指标

- 平均功能达成轮次：原始交互轮次未完整记录，无法精确统计。
- 每次 Prompt 的字数：原始 Prompt 未全部保存。
- AI 连续两次错误后的人工纠偏比例：缺少逐轮对话和完整错误输出。
- 精确的优化类 Prompt 占比：只能看到日志摘要，不能还原全部 Prompt 类型。
- 严格意义上的死循环解脱率：日志证明了问题闭环，但不能证明 AI 连续错误了几次。

### 允许使用的代理指标

| 代理指标 | 当前值 | 统计口径和限制 |
| --- | --- | --- |
| 已记录缺陷关闭率 | 7/7，按已记录缺陷为 100% | 该数据是缺陷闭环情况，不等同于完整原始对话计算出的死循环解脱率 |
| 修复/重构/测试/安全/隔离任务条目占比 | 至少 10/17，约 58.8% | 只统计日志中明确出现相关目标或记录的条目，未把所有普通功能开发都算入 |
| 自动化测试通过率 | 最新精确记录中，已执行测试为 `67 passed`、`0 failed`；若把 skipped 也计入总条目，则为 67/68 | Step 15 最终精确测试数量未写入日志，因此不能声称当前最终测试数量仍为 67 |

## 6. Step 记录格式说明

后文保留原始 Step 事实轨迹，不把后续修复合并伪装为首次完成。各 Step 可按以下字段阅读：

- 任务信息：日期、工具、目标和前置状态。
- 任务约束摘要：工作目录、技术栈、允许修改范围、禁止事项、预期产出和验收方式；这些均为根据现有日志整理出的摘要，不是原始 Prompt 原文。
- 实施内容：新增或修改文件、核心实现、关键设计决策、未采用方案及原因。
- 调试、纠偏与代码审查：只记录日志中确实出现的问题，不编造返工轮次。
- 验证结果：保留真实命令、`passed`、`failed`、`skipped` 状态，以及自动化验证和人工待验证的区分。
- 安全与风险：保留未验证事项、外部环境依赖和安全边界。
- 版本控制：以日志记录和当前只读 Git 核对为准；本次整理未创建 commit、tag 或 push。
...
- [Step 5 补充修复 A：编号系统修复](#step-5-补充修复-a编号系统修复)
- [Step 5 补充修复 B：隐藏数据库主键展示](#step-5-补充修复-b隐藏数据库主键展示)

## Step 5 补充修复 A：编号系统修复
...
- 原始现象：预设列表中用户看到的编号直接显示为数据库主键，例如内置预设从 8 开始。
- 风险：用户可能误以为菜单、用户、预设共用同一编号系统，或误把不连续主键当成异常。
- 根本原因：TUI 展示层直接把 `preset.id` 当作用户选择编号。
- 人工或任务方向调整：在专项检查或回归测试中发现后，明确区分“数据库 ID”和“展示序号”。
- AI 执行的修复：用户列表、预设列表、自定义预设编辑/删除列表使用从 1 开始的局部序号，并映射回真实数据库 ID。
- 验证命令：`uv run python -m compileall src scripts`、`uv run python scripts/init_db.py`、`uv run python src/main.py`、`uv run ruff check src scripts tests`、`uv run pytest`。
- 最终结果：回归验证通过；TUI 不再要求用户输入数据库 ID。
- 对应能力：需求控制 / 调试 / 抗幻觉 / 代码审查。

## 案例：UI 直接访问 backend 的分层重构

- 原始现象：会话管理存在 UI 直接访问存储后端的分层穿透风险。
- 风险：UI、业务、存储边界混乱，后续替换 MySQL/FileBackend 或增加 WebUI 时容易回归。
- 根本原因：部分会话管理逻辑没有集中到 `SessionManager`。
- 人工或任务方向调整：在专项检查或回归测试中发现后，将会话归属校验、加载、重命名和删除收口到业务层。
- AI 执行的修复：扩展 `SessionManager`，TUI 只使用临时展示序号和业务接口。
- 验证命令：`Get-ChildItem src\ui -Recurse -Filter *.py | Select-String "\.backend\."`、`uv run pytest`、`uv run ruff check .`。
- 最终结果：UI backend 访问检查无输出，测试通过。
- 对应能力：需求控制 / 代码审查 / 抗幻觉。

## 案例：模型配置来源统一

- 原始现象：用户默认模型、新建会话模型和 ChatEngine 实际模型来源不一致。
- 风险：用户看到的模型名与真实调用模型不一致，导致排查困难。
- 根本原因：配置来源分散，旧用户或旧会话记录可能保留历史值。
- 人工或任务方向调整：统一后续新建用户和新建会话的模型来源，但不自动迁移旧数据。
- AI 执行的修复：将用户默认模型和新建会话模型统一为 `config.model_name`，并补充检查。
- 验证命令：`uv run pytest`、`uv run ruff check .`、配置源检查。
- 最终结果：确认 `config.model_name` 与 `ChatEngine.model_name` 一致；旧数据影响被明确记录。
- 对应能力：需求控制 / 调试 / 代码审查。

## 案例：测试禁止读取真实 `.env`

- 原始现象：旧 ChatEngine 或 SessionManager 测试可能读取真实 `get_config()`、真实 `.env` 或构造真实模型。
- 风险：单元测试不可重复，可能泄露本地环境差异，甚至触发真实 LLM。
- 根本原因：测试边界没有与外部配置和模型服务隔离。
- 人工或任务方向调整：默认测试必须无外部依赖。
- AI 执行的修复：测试配置使用 `_env_file=None`，通过 monkeypatch 设置测试环境变量，在 `_get_model` 边界替换 `FakeModel`。
- 验证命令：`uv run pytest -q`、`uv run ruff check tests`、`uv run pytest --cov=src --cov-report=term-missing`。
- 最终结果：最终精确记录为 `67 passed, 1 skipped`；默认测试不调用真实 LLM，不连接真实 MySQL。
- 对应能力：调试 / 抗幻觉 / 代码审查 / 安全。

## 案例：prod MySQL 配置不得错误回退

- 原始现象：prod 使用 `PROD_MYSQL_*` 时，旧版 MySQL 环境解析可能使用通用 `EnvSettings` 默认值作为回退。
- 风险：缺失生产主机、用户或数据库变量时被默认值掩盖，造成错误连接或安全边界混淆。
- 根本原因：环境变量解析规则没有严格区分旧 `MYSQL_*` 和 prod 专属 `PROD_MYSQL_*`。
- 人工或任务方向调整：prod 环境必须显式提供生产变量，不允许回退 dev 或旧 `.env`。
- AI 执行的修复：限制旧默认值只适用于旧 `MYSQL_*` 名称；新增配置隔离测试。
- 验证命令：Step 15 新增 `tests/test_config_manager.py` 覆盖 prod 缺失 MySQL 配置失败且不泄露测试密钥。
- 最终结果：缺陷已记录并关闭；真实 prod MySQL 冒烟仍需外部环境验证。
- 对应能力：调试 / 抗幻觉 / 代码审查 / 安全。
...
- 项目骨架、Python 3.12.13 环境、配置样例和 Git 忽略规则。
- Pydantic 数据模型、`StorageBackend` 抽象接口、基础 TUI 主菜单。
- SQLiteBackend、五张业务表、初始化脚本和基础存储冒烟。
- UserManager、PresetManager、SessionManager 的核心业务逻辑。
- 内置预设幂等导入、自定义预设隔离和只读内置预设。
- 无状态 ChatEngine、非流式/流式调用接口、Token usage 处理和示例脚本。
- 会话列表、加载、重命名、删除、搜索、记录查看、导出和模型切换的业务实现。
- MySQLBackend 实现和默认跳过的显式集成测试入口。
- FileBackend、JSONL 结构化日志、日志脱敏和文件写入安全处理。
- 核心模块测试套件，日志中最新精确记录为 `67 passed, 1 skipped`。
- README、架构文档和后期 UI 能力协议预留。
- dev/test/prod 多环境配置隔离和 prod 不回退 dev 规则。

## 已实现但仍需人工或外部环境验证

- 完整 TUI 人工流程，包括用户、预设、会话、对话、搜索、导出和模型切换的连续操作。
- 真实 LLM 调用在当前最终状态下的可用性和生产配置。
- 真实 MySQL 服务、MySQL 集成测试和 `storage.type=mysql` 下的完整业务流程。
- prod 环境初始化、核心功能冒烟和跨环境真实业务流程。
- `.env.dev`、`.env.test`、`.env.prod` 在目标机器上的真实配置检查。
...
- 图文上传或文件解析。
- 语音输入输出。
- Agent 或 Tool Calling 执行逻辑。
- 上述后期功能在 Step 14 中仅预留接口或协议，没有新增 TUI 菜单入口，也不能写成已完成功能。

> 当前项目主体功能、工程化配置和自动化测试已经基本完成，但不能表述为所有生产环境和端到端场景均已完成验收。
